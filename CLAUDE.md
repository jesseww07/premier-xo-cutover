...
- **`Internal Server Error` from the CLI = the entryForm payload is unacceptable**, not a network blip - it prints before validation reports a single step. Three known causes, all bisected: reordering a field *inside* a NetSuite-standard field group (fields arriving from elsewhere are fine); `<visible>F</visible>` on the `ITEMMATRIX` subtab (remove the element instead; NetSuite re-adds it on install anyway); and clearing `<mandatory>` on a natively-required field such as `TAXSCHEDULE` (hiding it is fine, un-requiring it is not).
- **After `project:adddependencies` on the ACP, delete the `CHARGEBASEDBILLING` and `SUBSCRIPTIONBILLING` features** it adds - neither is enabled, and they fail ACCOUNT_SETTINGS_VALIDATION. `MATRIXITEMS` and `MERCHANDISEHIERARCHY` are forbidden too (Premier runs neither); they stay out as long as the form carries no matrix / merchandise-hierarchy elements. `scripts/check_acp.py` fails on all four.
- **Bulk item writes** (backfills, mass updates): disable `FA | UE Sync - Update FA`, `Update FA Map/Reduce`, `Update NS Map/Reduce` first.
- **Relay rules:** never write to XO (GET only; `relay/tests/test_readonly_guard.py` enforces it); never write `externalid` (LA's match key until 2026-10-01); NetSuite does not call XO directly (static-IP allowlist vs Oracle's 41 rotating egress IPs - decided); Phase 1 emits CSVs for the saved import map, Phase 2 REST writes only after several clean cycles in SB1.
