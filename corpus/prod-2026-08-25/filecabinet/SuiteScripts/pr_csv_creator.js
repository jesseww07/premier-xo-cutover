...
define(['N/config', 'N/https', 'N/log', 'N/record', 'N/render', 'N/runtime', 'N/search', 'N/url', 'N/https', 'N/encode', 'N/file'],
    ...
    function (config, https, log, record, render, runtime, search, url, https, encode, file) {
        ...
        function onRequest(ctx) {
            var fileContents = useHttpsModule('https://data52.com/AutoFeed/shoppremier.com/viqat2uplbapljithevl/netsuite-all.csv')
            log.debug('fileContents', fileContents)
            var delimiter = ",";
           // var dataArray = CSVToArray(fileContents, delimiter);
            var returnContents = CSVParser().parse(fileContents);
            log.debug('returnContents', returnContents);
            var dataArray = returnContents.data
            // log.debug('obj', obj);
            // var arr = obj[0]
            // log.debug('arr', arr);
            var arr = dataArray[7]
            ...
            var lineCount = dataArray.length
            log.debug('lineCount', lineCount)
            // var perTwentyThousand = Number(lineCount) / 10000
            // var arrayCount = Math.ceil(perTwentyThousand)
            ...
            for (var x = 0; x < dataArray.length; x++) {
                //for (var x = 0; x < 10; x++) {
                var dataDisplaySplit = dataArray[x]
                if(x == 7){
                    log.debug('dataDisplaySplit',dataDisplaySplit)
                    log.debug('t',dataDisplaySplit[51])
                }
                ...
                if (x == lastIndex) {
                    log.debug('last line - LINE', x)
                    log.debug('last line - counter', counter)
        ...
        const csvHeader = (peservedLineOne) => {
            var header = ''
            var stringLength = peservedLineOne.length
            var lastIndex = Number(stringLength) - 1
            for (var x = 0; x < peservedLineOne.length; x++) {
                if (x != lastIndex) {
                    header += peservedLineOne[x] + ','
                }
                else {
                    header += peservedLineOne[x] + '\n'
                }
        ...
        const csvCreator = (arraySet, peservedLineOne) => {
            ...
            var fileObj = file.create({
                ...
                contents: buildCSV,
                description: 'This is a test csv file.',
                folder: 391,
                isOnline: true
            });
        ...
        const preservecsvCreator = (arraySet, peservedLineOne) => {
            var content = ''
            var stringLength = arraySet.length
            var lastIndex = Number(stringLength) - 1
            for (var x = 0; x < stringLength; x++) {
                //for (var x = 0; x < 2; x++) {
                var lineInfo = arraySet[x]
                //var clearWrongfulParse = removeCommaIssues(lineInfo)
                //log.debug('lineInfo', lineInfo + ' - ' + x)
                var pushLine = ''
                var lastLineIndex = Number(lineInfo.length) - 1
                //log.debug('lineinfo lengh', lineInfo.length)
                for (var b = 0; b < lineInfo.length; b++) {
                    if (lastLineIndex != b) {
                        var cellInfo = lineInfo[b]
                        //log.debug('cellInfo', cellInfo)
                        pushLine += cellInfo + ','
                        //log.debug('cellInfo', cellInfo)
                    }
                    else {
        ...
        const removeCommaIssues = (lineInfo) => {
            //log.debug('lineInfo', lineInfo)
            var newArray = new Array()
            for (var x = 0; x < lineInfo.length; x++) {
                var cellInfo = lineInfo[x]
                var startsWithSpace = cellInfo.startsWith(' ')
                //log.debug('startsWithSpace', startsWithSpace)
                if (startsWithSpace) {
                    var lastItem = newArray[newArray.length - 1]
                    //log.debug('lastItem', lastItem)
                    newArray.pop()
                    var newItem = lastItem + cellInfo
                    newArray.push(newItem)
                }
                else {
                    newArray.push(cellInfo)
                }
            }
            //log.audit('newArray', newArray)
            return newArray
        }
