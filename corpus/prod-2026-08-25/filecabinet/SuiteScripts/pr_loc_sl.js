        ...
        function onRequest(context) {
            ...
            if (context.request.method === 'GET') {
                ...
                var sublist = form2.addSublist({
                    ...
                    label: 'Unordered Items for ' + vendorText + ' at Different Location'
                });

                var loca = form2.addField({
                ...
                var phx = form2.addField({
                    id: 'custpage_phx',
                    type: serverWidget.FieldType.TEXT,
                    label: 'PHX Value'
                });
                phx.updateDisplayType({ displayType: serverWidget.FieldDisplayType.INLINE });

                var sdl = form2.addField({
                    id: 'custpage_sdl',
                    type: serverWidget.FieldType.TEXT,
                    label: 'SDL Value'
                });
                sdl.updateDisplayType({ displayType: serverWidget.FieldDisplayType.INLINE });

                var tuc = form2.addField({
                    id: 'custpage_tuc',
                    type: serverWidget.FieldType.TEXT,
                    label: 'TUC Value'
                });
                tuc.updateDisplayType({ displayType: serverWidget.FieldDisplayType.INLINE });
sublist.addMarkAllButtons()
                var returnedLocationValues = getLocationInfo(vendor)
                if (returnedLocationValues) {
                    if (returnedLocationValues.length > 0) {
                        for (var a = 0; a < returnedLocationValues.length; a++) {
                            log.debug('returnedLocationValues[a]', returnedLocationValues[a])
                            if (returnedLocationValues[a].location == 2 || returnedLocationValues[a].location == 3) {
                                //Tuscon
                                log.debug('TuSCON SET', returnedLocationValues[a].location)
                                log.debug('TuSCON SET', returnedLocationValues[a].value)
                                tuc.defaultValue = returnedLocationValues[a].value
                            }
                            else if (returnedLocationValues[a].location == 8) {
                                //Phoen
                                log.debug('Phoen SET', returnedLocationValues[a].location)
                                log.debug('Phoen SET', returnedLocationValues[a].value)
                                phx.defaultValue = returnedLocationValues[a].value
                            }
                            else if (returnedLocationValues[a].location == 5 || returnedLocationValues[a].location == 6) {
                                //Scott
                                log.debug('Scott SET', returnedLocationValues[a].location)
                                log.debug('Scott SET', returnedLocationValues[a].value)
                                sdl.defaultValue = returnedLocationValues[a].value
                            }
                        }
                    }
                }
                ...
                for (var j = 0; j < requestCount; j++) {
                    ...
                    if (selected == 'T') {
                        ...
                        var returnedLocation = runDocSearch(parentDoc, chosenLocatioon)
                        log.debug('returnedLocation', returnedLocation + 'off of ' + parentDoc)
                        if (returnedLocation) {
                            log.debug('WE MADE IT IN', returnedLocation)
                            setParent = returnedLocation
                            //return
                        }
                    }
                }
                for (var x = 0; x < requestCount; x++) {
                    ...
                    if (selected == 'T') {
                        ...
                        objLocation.parentLine = setParent
                ...
                if (custArray) {
                    for (var c = 0; c < custArray.length; c++) {
                        ...
                        var customRecord = mergeLocation(parent, child)
                        log.debug('customRecord', customRecord)
                    }
                }
                var recalcSuccess = recalculateTotals(setParent)
        ...
        const recalculateTotals = (setParent) => {
            ...
            customrecord_zastro_po_consolidSearchObj.run().each(function (result) {
                ...
                var docCheck = result.getValue({
                    name: 'internalid'
                })
                var returnedSubSearch = checkChildRecords(docCheck)
        ...
        const mergeLocation = (parent, child) => {
            ...
            var customrecord_zastro_po_consolidSearchObj = search.create({
                ...
                columns:
                    [
                        ...
                        search.createColumn({
                            name: "custrecord_zastro_total_price",
                            summary: "GROUP"
                        })
                    ]
            });
        ...
        const runDocSearch = (parentDoc, masterLocation) => {
            var parentObj = record.load({
                type: 'customrecord_zastro_po_consolid',
                id: parentDoc,
                isDynamic: true
            ...
            var customrecord_zastro_po_consolidSearchObj = search.create({
                ...
                filters:
                    [
                        ...
                        ["custrecord_zastro_is_consolidated", "is", "F"],
                        "AND",
                        ["custrecord_ill_location", "anyof", masterLocation]
                    ],
                columns:
                    [
                        "internalid"
