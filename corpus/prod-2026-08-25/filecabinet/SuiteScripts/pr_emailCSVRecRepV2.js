...
define(["N/log", "N/record", "N/search", "N/util", "N/file", "N/email"],
    ...
    function (log, record, search, util, file, email) {
              ...
              //  let newReportDate = '4/4/2025'
                ...
                if (newReportDate) {
                    ...
                    for (let key in allIrDetailsCreatedToday) {
                        let createdFromOne = allIrDetailsCreatedToday[key][0].irCreatedFromId
                        let createdFromTextOne = allIrDetailsCreatedToday[key][0].irCreatedFrom
                        log.debug('createdFromOne', createdFromOne)
                        log.debug('createdFromTextOne', createdFromTextOne)
                        let transactionLookup = search.lookupFields({
                            type: "transaction",
                            id: allIrDetailsCreatedToday[key][0].irCreatedFromId,
                            columns: ["custbody_zastro_po_source", "createdfrom"]
                        });
                        ...
                        let createdFromSalesOrdText = transactionLookup.createdfrom ? transactionLookup.createdfrom[0] ? transactionLookup.createdfrom[0].text : "" : "";
                        let createdFromSalesOrd = transactionLookup.createdfrom ? transactionLookup.createdfrom[0] ? transactionLookup.createdfrom[0].value : "" : "";
                        log.audit('createdFromSalesOrdText', createdFromSalesOrdText)
                        log.audit('createdFromSalesOrd', createdFromSalesOrd)
                        let salesOrdCust = ''
                        if (createdFromSalesOrd) {
                            let salesOrdLookup = search.lookupFields({
                                type: "transaction",
                                id: createdFromSalesOrd,
                                columns: ['entity']
                            });
                            salesOrdCust = salesOrdLookup.entity ? salesOrdLookup.entity[0] ? salesOrdLookup.entity[0].text : "" : "";
                        } else if(consolidatedPoId){
                            let consolResults = consolSearch(consolidatedPoId,allIrDetailsCreatedToday[key][0])
                          if(consolResults.length>0){
                            log.audit('consolResults',consolResults)
                            let results = consolResults[0]
                            salesOrdCust = results.customer
                            createdFromSalesOrdText = results.salesOrder
                          }else{
                                salesOrdCust = null
                            createdFromSalesOrdText = null
                          }
                            
                            //data fr consolidated from search results
                            ...
                            //     columns: ['entity']
                            // });
                            // log.debug('entityLookup', entityLookup)
                            // salesOrdCust = entityLookup.entity ? entityLookup.entity[0] ? entityLookup.entity[0].text : "" : "";
                        }
                        
                        escapeCSV(salesOrdCust)
                        log.audit('salesOrdCust', salesOrdCust)
                        ...
                        allIrDetailsCreatedToday[key].map(allIrDetailsCreatedTodayData => {
                            log.audit('allIrDetailsCreatedTodayData', allIrDetailsCreatedTodayData)
                            let useItem = escapeCSV(allIrDetailsCreatedTodayData.itemName)
                            csvReportDataBody += `"${allIrDetailsCreatedTodayData.irNumber}","${allIrDetailsCreatedTodayData.irQty}","${allIrDetailsCreatedTodayData.irCreatedFrom}","${useItem}","${allIrDetailsCreatedTodayData.inboundShip || ""}","${salesOrdCust || ""}",${createdFromSalesOrdText || ""},"${allIrDetailsCreatedTodayData.irQty}","${allIrDetailsCreatedTodayData.location}","${allIrDetailsCreatedTodayData.binNumber}",\r\n`
                        })
                    };
        ...
        function consolSearch(consol,dataObj){
            log.audit('in consolSeaarch', consol)
            ...
            var customrecord_zastro_unconsolidated_itemsSearchObj = search.create({
                ...
                filters:
                [
                   ["custrecord_zastro_po_item_list","anyof",consol], 
                   "AND", 
                   ["custrecord_zastro_item_name.internalidnumber","equalto",dataObj.itemId], 
                   "AND", 
                   ["custrecord_zastro_qty","equalto",dataObj.irQty]
                ],
                columns:
                [
             ...
             customrecord_zastro_unconsolidated_itemsSearchObj.run().each(function(result){
                let returnObj = {}
                let customer = result.getText('custrecord_zastro_customer')
                let salesOrder = result.getText('custrecord_zastro_so_no')
        ...
        function escapeCSV(str) {
            // Check if the string is not null and is a valid string
            if (str == null) {
                return '';
            }

            // Convert the string to a string (in case it is an object)
            str = String(str);

            // Escape double quotes by doubling them
            str = str.replace(/"/g, '""');

            // If the string contains commas, newlines, or double quotes, surround it with double quotes
        ...
        const buildCSV = (fileObj, emailArray) => {
            ...
            email.send({
             ...
             //   recipients: 'mccoy@zastro.com',
                subject: 'UPDATED V2 Daily Receiving Report' + ' - ' + today,
