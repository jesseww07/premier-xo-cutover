 ...
 function (config, https, log, record, render, runtime, search, url, format) {
     ...
     const getDeliveryContents = (id) => {
         ...
         var customrecord_stored_inventory_contentsSearchObj = search.create({
          ...
          customrecord_stored_inventory_contentsSearchObj.run().each(function(result){
             ...
             var instructions = result.getText({
                 name: 'custrecord_pr_deliver_instructions'
             })
             var returnInfo = getAdditionalInfo(id, itemId)
             
             var returnObject = new Object()
             returnObject.bin = bin
             ...
             returnObject.shipdate = returnInfo.shipdate
             returnObject.shipmethod = returnInfo.shipMethod
             returnObject.description = returner(returnInfo.desc)
             returnObject.soText = soText
             returnObject.roomLocation = returner(roomLocation)
            returnObject.deliveryID = id
            returnObject.instructions = returner(instructions)
    ...
    var instPay = search.lookupFields({
       ...
       var instructions = instPay.custrecord_pr_deliver_instructions
         ...
         xml += `</td>`
         xml += `<td align="center" style="width: 60%; padding: 0;"><span style="font-size: 28pt;">Stored Delivery Slip</span></td>`
  
     
         xml += `<td style="width: 20%;">`
         xml += `<p style="align: right; text-align: right">`
         xml += `Delivery Record: ${id}</p>`
         xml += `</td>`
         xml += `</tr>`
         xml += `<tr>`
         ...
         xml += `</tr>`
         xml += `<tr>`
         xml += `<td colspan="1"></td><td colspan="10" style="font-size: 18px; align: center">${record[0].customer}</td><td colspan="3"></td>`
         xml += `</tr>`
         xml += `<tr>`
         ...
         xml += `<tr>`
         xml += `<td>`
         xml += `<barcode codetype="code128" showtext="true" value="${record[0].salesOrd}" />`
         xml += `</td>`
         xml += `<td align="right">`
         xml += `<pagenumber /> of`
         ...
         xml += `<body header="nlheader" header-height="16%" footer="nlfooter" footer-height="20pt" padding="0.5in 0.5in 0.5in 0.5in" size="Letter">`

         xml += `<table>`
         xml += `<tr>`
              xml += `<td style="line-height: 150%; background-color:#ffff99" colspan="5"><b>Special Instructions:</b> <br />${instructions}</td>`
              xml += `</tr>`
              xml += `</table>`
         xml += `<table align="center" class="body" style="width: 100%; margin-top: 5px;">`
         xml += `<tr>`
         xml += `<td colspan="3"></td><td colspan="10" style="font-size: 18px; align: center"></td><td colspan="3"></td>`
         xml += `</tr>`
         ...
         xml += `<th align="right">Ship Via</th>`
         xml += `<th>Estimated <br /> Ship Date</th>`
         xml += `<th>Sales Rep</th>`
         ...
         xml += `<th colspan="2">Qty.</th>`

         xml += `<th colspan="6">Sales Order</th>`
     ...
     for(var x=0;x<record.length;x++){
         ...
         xml += `<td style="line-height: 150%" colspan="5">${record[x].roomLocation}</td>`
         xml += `<td style="line-height: 150%" colspan="5">${record[x].bin}</td>`
     
         xml += `<td style="line-height: 150%" colspan="2">${record[x].qty}</td>`

         xml += `<td style="line-height: 150%" colspan="6">${record[x].soText}</td>`
         xml += `<td style="border: 1px solid black; align: right;" colspan="1"></td>`
         xml += `<td style="border: 1px solid black; align: right;" colspan="1"></td>`
         xml += `<td style="border: 1px solid black; align: right;" colspan="1"></td>`
